...
首先要理解 useState，我们要解决一个核心问题：**函数组件本身是无状态的**，它每次渲染都是一次新的函数调用，执行完后所有局部变量都会丢失。那么，React 是如何在多次渲染之间“记住”这个 state 的呢？
...
1. **数据结构**：在每个组件的 Fiber 节点上，React 维护了一个**链表**，这个链表专门用来**存储所有的 Hooks**（包括 `useState`, `useEffect` 等）。
