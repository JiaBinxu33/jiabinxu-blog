...
- react 内部运行机制串联起来
  首先 react 内部核心的机制叫虚拟 dom，通过数据来模拟真实 DOM 节点，从而实现每次修改 dom 的时候有一个缓冲，可以通过改变数据来间接操作真实 dom，这个模拟的数据架构叫做 fiber 架构，它实质上是一个链表，这个 fiber 架构内部的时间分片机制让 react 内部可以中断，把一个大的渲染任务分成多个片，每个片执行完之后可以暂停，中断完之后还可以继续执行(如果 React 在暂停期间，接收到了一个**更高优先级的更新**（比如用户的紧急点击输入），React 并不会傻傻地接续执行，而是会**直接废弃掉之前做了一半的低优先级旧工作**，转而去处理紧急任务，处理完后再重新开始计算那颗低优先级的树。)，当数据更新啊的时候 react 会通过 diff 算法来处理新的 fiber 树和旧的 fiber 树之间的区别，看哪里发生了变化，diff 算法的核心首先是同层比较，这样只比一层的话直接把时间复杂度降成 on 了，然后是 key 的比较，在同一层中每个节点都有对应的 key 通过这个 key 观察是否发生了变动，还有节点类型的比较，如果节点类型相同的 React 会认为他可以复用。它会保留 DOM 节点或组件实例，只更新变化的 `props` 和 `style`。类型不同 React 会**彻底销毁**旧的组件并**创建**一个全新的组件实例
...
- **核心思想：** Web UI 很少会发生“跨层级”的节点移动。比如你在 div 下的节点通常不会直接移动到 body 下
- **工作机制：**
  1. React 的 diff 过程是通过逐层比较。
  2. 它在比较两棵虚拟 DOM 树时，只会比较**同一层级**的节点。
- **目的/作用：** 这是最大的性能优化。React **放弃了“跨层级比较”**，以减少算法复杂度。
- **如何处理跨层级：**
  - 它会在**旧层级销毁**这个节点（触发 `unmount`），然后在**新层级创建**一个全新的节点（触发 `mount`）。
  - 这意味着，即使是同一个组件，只要跨层级移动了，它的所有 `state` 都会丢失。

---

### 组件类型比较

- **核心思想：** 不同类型的组件，会产生不同的 DOM 结构。
- **工作机制：**
  1. 当 React 在**同一层级**比较两个节点时，它首先会看它们的“类型”。
  2. **如果类型相同**（比如都是 `div`，或者都是 `MyButton` 组件）：React 会认为他可以复用。它会保留 DOM 节点或组件实例，只更新变化的 `props` 和 `style`。
  3. **如果类型不同**（比如 `div` 变成了 `span`）：React 认为它们“毫无关系”。
- **目的/作用：** 这样避免在两个完全不相干的组件上浪费时间去比较它们的内部细节。
- **如何处理不同类型：**
  - 当类型不同时，React 会**彻底销毁**旧的组件（及其所有子节点），并**创建**一个全新的组件实例（及其子节点）。
  - 这同样意味着，旧组件的所有 `state` 都会丢失。

---

### key 的比较

- **核心思想：** 在一组子节点中，使用 `key` 属性来**唯一标识**每一个节点。
- **工作机制：** 当 React 比较**同一层级**的**一组**子节点时（比如 `ul` 下的多个 `li`）：
  - **没有 key (旧策略)**：React 会“逐个对比”。如果我在列表**最前面**插入一个新节点，React 会误以为“所有”节点都发生了变化（第 1 个变第 2 个，第 2 个变第 3 个...），导致**所有节点都被原地更新**（或重建），性能极差。
  - **有 key (新策略)**：React 会利用 `key` 创建一个 `Map`（`key` -> 节点）。
    1. 它遍历新列表，通过 `key` 去旧的 `Map` 里查找。
    2. **找到了 (key 存在)**：说明这个节点只是**移动了**位置，React 会复用它并执行“移动”操作。
    3. **没找到 (key 是新的)**：说明这是**新增**节点，React 会创建它。
    4. (遍历完新列表后) 旧 `Map` 里还**剩下**的 `key`：说明这些是**删除**节点，React 会销毁它们。
- **目的/作用：** 实现 DOM 节点的**最大化复用**。它能让 React 精确地知道哪些节点是“新增”、“删除”或“移动”的，从而执行最少的 DOM 操作。
- **关键点/陷阱：**
  1. `key` 必须在**兄弟节点之间唯一**（不需要全局唯一）。
  2. **绝对禁止使用数组的 index 作为 key**。因为当列表发生“反转”或“在开头插入”时，`index` 会完全错乱，这会导致 React 错误地复用组件，引发各种 `state` 混乱的 Bug，其危害甚至比不写 `key` 更大。

## Fiber 的核心机制

React 15 依赖 JavaScript 的“函数调用栈”来递归 diff，这个过程一旦开始就无法停止。

Fiber 架构在内存中构建了一个**链表**（即“虚拟栈”），React 通过遍历这个链表来执行工作，从而**避免阻塞浏览器主线程**,React 会在一个时间片（通常是 5 毫秒左右）内执行计算。如果时间片用完，React 会主动让出（Yield）主线程的控制权，让浏览器优先处理更高优先级的任务（如 UI 绘制、响应用户输入）。

Diff 算法 (Reconciliation)： 是 “要做什么”。它是一套比较规则，用来找出新旧两棵树的差异。

Fiber 架构： 是 “要怎么做”。它是 React 16+ 引入的一套全新的“执行引擎”或“调度器”，用来执行这个 Diff 算法。

### 可中断性

- **它是什么：** React 能够将一个“大更新”（如 diff 整棵树）拆分成无数个“小工作单元”，并在执行完每个单元后暂停。
- **如何实现：**
  - **Fiber 单元：** 树中的每个节点（组件、DOM）都是一个 **Fiber 对象**，即一个“工作单元”。
  - **链表指针：** 每个 Fiber 对象包含三个关键指针：`return` (父级), `child` (子级), `sibling` (兄弟)。
  - **遍历：** React 的调度器**在内存中**通过这三个指针“遍历”Fiber 树（而不是通过 JS 函数调用）。
  - **实现中断：** 因为是 React 自己控制的“遍历”，所以它可以在处理完**任何一个** Fiber 单元后，**主动停止**，并把主线程交还给浏览器（去响应用户输入）。这就是“时间分片”。

### 断点重新执行

- **它是什么：** “中断”之后，还能在稍后从“断点”处**继续**工作，而不是重头再来。
- **如何实现：**
  - 这正是“链表遍历”带来的好处。
  - 当 React 决定“中断”（比如 5ms 时间到了）时，它只需要**在内存中记住**它“**下一个**要处理的 Fiber 单元”的**指针** (`nextUnitOfWork`)。
  - 当浏览器再次空闲时，React 调度器直接从这个指针指向的 Fiber 单元**继续开始**遍历和 diff，之前的所有工作进度（即那棵“WIP 草稿树”）都完整地保存在内存中。

### 优先级

- **它是什么：** React 必须能区分“紧急任务”（如用户打字）和“非紧急任务”（如渲染列表），并允许“紧急任务”**插队**。
- **如何实现：**
  - **调度器 (Scheduler)：** React 18 引入的机制，用于给不同的更新“贴标签”（如：`Urgent`, `Transition`）。
  - **两阶段：** Fiber 架构将更新严格分为两个阶段：
    1. **阶段一：Render (可中断)**：执行 Diff，找出差异。
    2. **阶段二：Commit (不可中断)**：将差异应用到真实 DOM。
  - **实现插队：**
    - **场景：** React 正在“慢悠悠”地执行一个“非紧急”的 **Render** 阶段。
    - **插队：** 此时一个“紧急”的更新（用户点击）进来了。
    - **操作：** 调度器会**立即“丢弃”** 那个正在进行中的、非紧急的 Render 阶段（以及它的“草稿树”），然后**立即启动**一个新的 Render 阶段来处理这个“紧急”更新。
    - **提交：** 紧急更新的 Render 和 Commit 阶段会快速执行，保证用户响应。之后，React 再**重新**去执行那个被“丢弃”的非紧急更新。
...
- MVC 的通信方式： view -> controller -> model -> view
  ...
  - View 将用户操作转发给 Controller，Controller 执行业务逻辑并更新 Model 的状态，最后 Model 的数据变更触发 View 的被动刷新。
...
- MVVM 的通信方式： view <-> viewmodel -><- model
  - View 通过触发事件将操作指令传递给 ViewModel，ViewModel 接收后调用 Model ，Model 将处理结果返回给 ViewModel，最后 ViewModel 通过响应式的数据绑定自动更新 View。
...
- **代表库**:
  ...
  - **Mobx**: 一个更轻量、更现代的状态管理方案，API 简单，基于 Hooks，无需大量模板代码。
...
- 为什么文件上传一定要使用非受控组件？
  ...
  2. 受控组件的核心工作原理是 value={state}，由于文件输入框的 value 是路径 无法通过 JavaScript 去改变，所以 react 不能通过状态去控制它
...
- **它是什么：** 一个**高阶组件 (HOC)**，即一个用来“包裹”你组件的函数。
- **工作机制：**
  1. 你用 `React.memo` 包裹一个功能组件（`const MyMemoizedComponent = React.memo(MyComponent)`）。
  2. 当其**父组件**重新渲染时，`React.memo` 会“拦截”这次渲染。
  3. 它会**“浅比较”** (shallow compare) 这次传给 `MyComponent` 的 `props` 和上一次的 `props`。
  4. **决策：**
     - 如果所有 `props` **都** `===` (严格相等)，`React.memo` 就会**阻止 MyComponent 重新渲染**，并直接复用上一次的渲染结果。
     - 如果有**任何一个** `prop` 变了，它才会允许 `MyComponent` 重新渲染。
- **目的/作用：**
  - 它的**唯一目的**是：**防止组件因为“父组件更新”而导致的“被动”重新渲染**。
- **关键点 (类比)：**
  - `React.memo` 之于“函数组件”，就如同 `PureComponent` 之于“类组件”。

## React 18：启用 Fiber 架构

React 18 的核心主题是**“并发”**。它引入了一个可中断的渲染引擎 [fiber 架构](#fiber-的核心机制)，让 React 在处理复杂渲染时也能保持 UI 的流畅响应。

### 自动批处理 (Automatic Batching)

- **核心思想：** 把多次 `setState` 合并为一次渲染。
- **工作机制：** R17 只在 React 事件（如 `onClick`）中批处理。R18 将其扩展到了 `Promise`、`setTimeout` 等**任何异步**回调中。
- **目的/作用：** 减少不必要的重复渲染。

### 新 Hooks

- **useId**：解决 SSR (服务端渲染) 场景下，客户端和服务器端 ID 不匹配导致的“水合”错误。
- **useDeferredValue**：`useTransition` 的“数据”版本，用于推迟更新一个“非紧急”的值。

---

## React 19：编译器与新架构

React 19 的核心主题是**“大幅简化开发”**。它通过“编译器”自动化了性能优化，并通过“RSC 架构”重塑了我们编写和交付组件的方式。

### React 编译器

- **核心思想：** 从“手动优化”转向“自动优化”。
- **工作机制：** 这是一个构建时工具，它会自动分析你的组件代码。
- **目的/作用：** 编译器会自动帮你实现 `useMemo`、`useCallback` 和 `React.memo` 的功能。开发者**不再需要**（或极少需要）手动包裹这些 Hooks，极大简化了代码。

### Actions

- **核心思想：** 将“数据提交”和“状态管理”内置到 React 中。
- **工作机制：** 允许在 `<form>` 上使用 `action` 属性绑定异步函数，并配合新 Hooks (`useOptimistic`, `useFormStatus`) 自动管理 `pending`、`error` 等状态。
- **目的/作用：** 大幅简化表单和异步数据提交的模板代码。

### Server Components (RSC - 新架构)

这是 React 19 最大的架构变化，也是刚才我们讨论的重点，它与 SSR (服务端渲染) 是**完全不同**的概念。

- **它是一种“组件架构”，不是一种“渲染模式”**
  - 它把 React 组件**严格拆分**为两种类型。
- **A. 服务端组件 (Server Components - RSC)**
  - **这是默认的**（在 Next.js App Router 中）。
  - **工作机制：**
    1. 它**只在**服务端运行。
    2. 你可以**直接**在组件里用 JS/React 语法（`async/await`）访问数据库或后端资源。
    3. 它的 JS 代码**永远不会**发送到浏览器。
  - **关键点 (水合)：** 因为它的 JS 不会发送到客户端，所以 RSC **“永不水合” (No Hydration)**。它对于浏览器来说是纯静态的。
  - **限制：** 不能使用 `useState`, `useEffect`, `onClick` 等任何客户端 Hooks。
- **B. 客户端组件 (Client Components)**
  - **这是您写前端业务逻辑的地方。**
  - **工作机制：**
    1. 你**必须**在文件顶部用 `"use client";` 标记。
    2. **所有**的 Hooks (`useState` 等) 和事件 (`onClick`) **必须**写在这里面。
  - **关键点 (水合)：** 客户端组件的 JS 代码**会**被发送到浏览器。
  - **它与 SSR 的关系：** Next.js 仍然会**对客户端组件执行 SSR**，即在服务器上预渲染它们，生成**静态 HTML** 发给浏览器（保证首屏速度），然后客户端下载 JS 包，执行**“水合” (Hydration)**，让这些 HTML 变得可交互。

---

### 总结：RSC 与 SSR 的协同工作

所以，在 React 19 和 Next.js App Router 的新模型下：

- **RSC (服务端组件)** 负责“骨架”和“数据获取”，它的 JS 留在服务器，**不水合**。
- **SSR (服务端渲染)** 负责预渲染那些“交互点”（`"use client";` 组件），生成 HTML。
- **Hydration (水合)** **只会**发生在那些 `"use client";` 的“群岛”上，以激活它们的交互性。

这是一个“混合”模型，它让我们的应用兼顾了“零 JS 体积”（RSC）和“丰富交互性”（Client Components）。

## 自定义 Hooks 的设计规范

- **它是一种模式，不是一个功能**：自定义 Hook 本质上只是一个**普通的 JavaScript 函数**。
- **它的特殊性**：这个函数特殊在它的“内部”调用了其他 React Hooks（如 `useState`, `useEffect` 或其他自定义 Hooks）。
- **它的命名**：它的名字**必须**以 `use` 开头（例如 `useWindowSize`）。

### 目的

- **复用“有状态的逻辑”**：这是它的唯一目的。
- **场景**：当你有多个组件都需要做同样的事情时（比如：订阅事件、获取数据、跟踪表单状态），你就可以把这套“包含了 state 或 effect”的逻辑，抽离到一个自定义 Hook 中。
- **保持组件简洁**：它能把复杂的逻辑从组件视图中剥离出去，让组件只关心 UI 渲染。
...
- **核心思想**：这是一个**命名约定**。
- **工作机制**：React 本身并不关心函数名叫什么。但是，**ESLint 插件 (linter)** 会“盯”这个 `use` 前缀。
- **目的/作用**：
  1. **代码可读性**：开发者一看 `use` 就知道：“哦，这里面有状态，它必须遵守 Hook 的规则”。
  2. **自动检查**：更重要的是，ESLint 插件会**自动帮你检查**你是否违反了“规范二”（在循环或 `if` 中调用）。如果你不以 `use` 开头，linter 就“瞎了”，无法帮你检查错误。

### 规范二：只能在“顶层”调用

这是 React 工作的**核心依赖**，也是最容易出问题的。

- **核心思想**：永远在你的 React 函数（或另一个自定义 Hook）的**最顶层**调用 Hooks。
- **绝对禁止**：
  - **禁止在 if 语句中调用**
  - **禁止在 for 或 while 循环中调用**
  - **禁止在普通 JS 函数中调用** (包括 `useEffect` 的回调、事件处理器如 `onClick` 等)

### 为什么必须在顶层？

这背后的原因是 React 内部**没有**用“名字”来识别 Hooks，它完全依赖于**“调用顺序”**。

- **工作机制**
  - React 在内部为每个组件维护了一个（概念上的）**数组**或**链表**，用来存储 Hooks 的状态。
  - **第一次渲染 (mount)**：
    1. 你调用 `useState("A")` -> React 把 "A" 存到**数组第 0 位**。
    2. 你调用 `useEffect(...)` -> React 把 effect 存到**数组第 1 位**。
    3. 你调用 `useState("B")` -> React 把 "B" 存到**数组第 2 位**。
  - **第二次渲染 (re-render)**：
    1. React 期待**第一个**被调用的 Hook 是“第 0 位的 `useState`”，于是它返回 "A" 的当前状态。
    2. React 期待**第二个**被调用的 Hook 是“第 1 位的 `useEffect`”。
    3. ...以此类推。
- **如果你违反了规则**
  - **场景**：假设你在**第二次**渲染时，用一个 `if` 语句**跳过**了第一个 `useState` 的调用。
  - **灾难**：
    1. React 期待的“第 0 位 `useState`”没来。
    2. 你调用的第一个 Hook 变成了 `useEffect`。
    3. React 以为这个 `useEffect` 是“第 0 位 Hook”，于是尝试从第 0 位取状态... **整个链表全部错位**，React 彻底混乱了。

**总结**：`use` 开头是给 linter 看的，而“顶层调用”是给 React 内部的“顺序记账本”机制看的。

---

### Hooks 复用的是逻辑，不是状态

- **核心思想**：每次你在组件中调用一个自定义 Hook，你都是在“复印”一份新的逻辑。
- **举例**：
  - `组件A` 调用了 `useMyHook()`。
  - `组件B` 也调用了 `useMyHook()`。
- **结果**：这两个组件各自拥有**一套完全独立、互不相干的状态**。`useMyHook` 内部的 `useState` 在组件 A 和组件 B 中是**完全隔离**的。
- **对比**：这和 Redux 或 Zustand 那种“共享全局状态”的模式是完全不同的。
