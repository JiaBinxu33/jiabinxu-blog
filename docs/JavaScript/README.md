...
- **async：异步标记与自动包装**
  - **语义标记**：告诉 JavaScript 引擎，该函数内部包含异步操作。
  - **底层包装**：`async` 函数的返回值始终会被自动包装成一个 Promise 对象。
- **await：数据拦截与同步化表达**
  - **数据拦截**：它本质上是一个“数据拦截器”，负责拦截本该进入 `.then(res => { ... })` 回调函数的决议结果 `res`。
  - **扁平化赋值**：利用类似同步赋值的语法，直接将拦截到的真实数据交到等号左边的变量手中，从而完美避免了“回调地狱”

```js
// 这是一个模拟接口”
function fetchNumber() {
  return new Promise((resolve) => {
    // 延迟一秒传递：使用定时器模拟 1 秒的网络请求耗时
    setTimeout(() => {
      // 这里的 100，就是最终会被提取出来，传递到下方 await fetchNumber() 位置的值
      resolve(100);
    }, 1000);
  });
}
...
async function run() {
  // 2️⃣ 核心流向：
  // await 接收到 Promise 后，会在这里“挂起”并等待 1 秒。
  const num = await fetchNumber();

  // 3️⃣ 1 秒后恢复执行，拿到真实数据
  console.log("拿到的数字是：", num); // 输出：拿到的数字是： 100
}
...
- **“值”的集合：** `Set` 是一种只存储“值”的集合。
- **唯一性：** 它的核心特点是，`Set` 内部的**所有值都是唯一的**。它像一个“不能有重复项”的数组。

- 作用

  - **保证数据唯一：** 它的主要用途是存储一系列**不需要重复**的值。
  - **数组去重：** 在实践中，它最常被用来为“数组”进行快速去重。
  - **快速查找：** 用来快速检查一个“值”是否存在于某个集合中（`has` 操作）。

- 用法

  - **键值相同：** 它的底层存储也是“键值对”的，但它的“键”和“值”是**相同**的。
  - **核心操作：**
    - `add` (添加)：添加一个新值。如果值已存在，则什么也不做。
    - `has` (检查)：检查一个值是否存在，返回 `true` 或 `false`。
    - `delete` (删除)：删除一个值。
    - `size` (大小)：获取集合中值的数量（类似数组的 `length`）。

- 注意

  - **唯一性判断：** `Set` 判断“值”是否相等，基本和 `===` (严格相等) 一致，但有一个**例外**。
  - **NaN 的处理：** `Set` 认为 `NaN` **等于** `NaN`。这意味着你向 `Set` 中 `add` 多个 `NaN`，最终只会保留一个。
  - **对象引用：** 如果存的是对象，`Set` 比较的是“内存地址”（引用）。两个内容相同的**不同对象**，在 `Set` 中被视为**两个不同**的值。
  - **无索引：** `Set` 是一个集合，**没有“下标”**（Index）。你不能通过 `set[0]` 这样的方式访问。
  - **迭代顺序：** `Set` 是**有序的**。它的迭代顺序（例如 `forEach`）就是**插入顺序**。

---

### Map (字典)

- **“键值对”的集合：** `Map` 是一种存储“键值对”（Key-Value Pair）的集合。
- **“超级对象”：** 它解决了 JavaScript 传统对象 (`Object`) 的一个核心限制。

- 作用

  - **解决 Object 的限制：** 传统 `Object` 的“键”**只能是字符串**或 `Symbol`。如果你尝试用一个“对象”作为键，它会被自动转成字符串（如 `[object Object]`）。
  - **任意类型的键：** `Map` 的“键”**可以是任意数据类型**，包括（但不限于）`Object`, `Array`, `Function`, `NaN`, `null` 等。
...
- 注意
  ...
  - **键的类型：** `Map` 真正强大的地方在于它对“键”的包容性。
  - **NaN 的处理：** `Map` 也认为 `NaN` 键**等于** `NaN` 键。
  - **迭代顺序：** `Map` 也是**有序的**。它的迭代顺序就是**插入顺序**。
  - **与 Object 的选择：**
    - **Object：** 当你只需要简单的、键是“字符串”或 `Symbol` 的哈希表时，用 `Object`。
    - **Map：** 当你需要“非字符串”作为键，或者你需要保证“插入顺序”时，必须用 `Map`。
  - **内存陷阱（WeakMap）：**
    - `Map` 会强引用（Strong Reference）它的“键”和“值”。
    - **陷阱：** 如果你用一个“对象”作为 `Map` 的键，只要这个 `Map` 存在，那个“对象”就**永远不会**被垃圾回收，这可能导致内存泄漏。
    - **解决：** 如果你只是想把“数据”临时“挂载”到某个对象上，并且希望该对象在别处被销毁时自动释放内存，应该使用 `WeakMap`。
...
1. 执行同步代码（宏任务）： 事件循环开始，首先会执行主线程上的同步代码。比如 console.log('script start') 会被立即执行。
...
4. 相等性比较
   ...
   在实际开发中，**始终推荐使用严格相等 (`===`)**，因为它能更精确地判断值和类型的真实情况。
...
2. 静态集合类（比如 Map, Set，如果不及时清理）

3. 资源未关闭（比如 WebSocket 连接、文件句柄）

4. 改变哈希值（可能影响 Map/Set 的查找和删除）

5. 缓存泄漏（缓存数据不断增长且没有清除策略）

6. 监听器和回调：
   当你为一个 DOM 元素或其他对象（比如事件发射器）添加了事件监听器（addEventListener）或注册了回调函数后，如果这个监听器/回调函数或者它内部引用的变量（通过闭包）持有对该 DOM 元素或其他不再需要的对象的引用，并且你忘记在适当的时候移除这个监听器/回调，就会发生内存泄漏

7. 不合理使用闭包

8. 函数柯里化（如果创建过多中间函数且未被释放）
...
- **for...in** 是为**遍历对象**而设计的，它遍历的是对象的**“键”（key）**（并且是字符串类型）。
- **for...of** (ES6 新增) 是为**遍历可迭代对象**（如数组、Map、Set、String）而设计的，它遍历的是**“值”（value）**。
...
| **特性**     | **for...in**                     | **for...of**                                        |
| ------------ | -------------------------------- | --------------------------------------------------- |
| **遍历目标** | 对象的**可枚举属性键 (key)**     | **可迭代对象**的**值 (value)**                      |
| **返回值**   | `string` (属性名或数组索引)      | 元素的值 (任意类型)                                 |
| **适用对象** | 任何对象 (最常用于普通 `Object`) | **可迭代对象** (如 `Array`, `String`, `Map`, `Set`) |
| **原型链**   | **会** 遍历原型链上的属性        | **不会** 遍历原型链                                 |
| **迭代顺序** | 不保证顺序 (尤其对对象)          | 按迭代器定义的顺序 (如数组索引)                     |
...
这是 `for...in` 最大的问题。它不仅会遍历对象**自身**的属性，还会遍历其**原型链**上所有可枚举的属性。
...
为了避免这个问题，当（且仅当）你必须使用 for...in 遍历一个普通对象时，必须在循环体内部使用 Object.prototype.hasOwnProperty.call() 或 myObject.hasOwnProperty() 来过滤掉原型链上的属性。
...
当 `for...in` 用于遍历数组时，它返回的索引 `i` 是**字符串** `'0'`, `'1'`, `'2'`... 这在进行数学运算时会导致错误。
...
规范没有保证 `for...in` 会按特定顺序（如数字索引顺序或属性插入顺序）返回键。虽然大多数现代浏览器会按顺序遍历数组索引，但在遍历普通对象时，顺序是不可靠的。
...
- **用 for...in**： **只用于**遍历一个普通的、非数组的 `Object`，目的是获取它的**键 (key)**，并且**必须**配合 `hasOwnProperty` 使用。
- **用 for...of**： 用于遍历**可迭代对象**（`Array`, `String`, `Map`, `Set`, `NodeList` 等）的**值 (value)**。这是遍历数组最现代、最推荐的方式（如果您不使用 `.forEach()` 或 `.map()` 的话）。

## 性能优化

### map 循环展示添加 key

- **原理：** `key` 是 React 用来追踪同级元素列表（list）中各个元素的“身份证”。在数据更新、列表发生变化时，React 的 Diff 算法会使用 `key` 来“匹配”新旧两个列表中的元素，以最小化 DOM 操作。
- **优点：** 提供了高效的 DOM-Diff。通过 `key`，React 可以准确地知道哪些元素是新增的、删除的、还是仅仅移动了位置。这避免了不必要的 DOM 重建（unmount/mount），保证了更新效率，并能正确保留组件的状态（如 `input` 框的输入内容）。
- **缺点：** 最大的“缺点”来自于**错误使用**。如果使用不稳定的 `key`（如 `Math.random()`）会导致性能更差。如果使用数组的 `index` 作为 `key`，在列表项顺序会改变（如排序、在中间插入/删除）的场景下，会导致 Diff 错乱，引发 Bug 和性能问题。

### 路由懒加载 (Route Lazy Loading)

- **原理：** 这是一种**代码切割 (Code Splitting)** 的核心应用。它利用了 ES 模块的**动态导入 import()** 语法。构建工具（如 Webpack/Vite）在打包时，会把动态导入的模块（即路由组件）分离成一个独立的 JS "chunk" 文件。
- **优点：** 显著减小了**主包（main bundle）的体积**。用户首次访问时，只需下载主包和当前路由的 JS 文件，无需一次性加载整个应用的所有页面代码，这使得**首屏加载速度（FCP/LCP）** 极大提升。
- **缺点：** 会带来一个微小的**“切换”成本**。当用户第一次跳转到某个懒加载的路由时，需要临时去网络请求该路由的 JS chunk，这会有一个短暂的加载延迟（通常会用 loading 动画覆盖）。

### 第三方插件按需引入

- **原理：** 现代前端库（如 Ant Design, ECharts, Lodash）通常会提供 ES Module (ESM) 规范的构建产物。按需引入（或称“摇树优化”，Tree Shaking）依赖于 ESM 的**静态分析**特性。构建工具在打包时，会分析 `import`/`export` 语句，只把代码中**实际使用到**的函数或模块打包进来，未被引用的代码（Dead Code）则会被删除。
- **优点：** 极大减少了打包体积，特别是对于大型的 UI 库或工具库。例如，只 import 一个 `Button` 组件，就无需打包整个 UI 库的其他几十个组件，性能提升明显。
- **缺点：** 这种优化**强依赖于库本身的支持**。如果一个库不是用 ESM 编写的，或者其代码有“副作用”（side effects），构建工具就很难安全地进行 Tree Shaking，导致优化失败。

### 使用 PureComponent/React.memo (SCU)

- **原理：** 这是 React 性能优化中的**“渲染节流”**手段。当父组件重新渲染时，默认会“递归”渲染其所有子组件。PureComponent (用于 Class) 和 React.memo (用于函数组件) 会在组件渲染前，自动对其新旧 props 进行一次**浅比较（Shallow Compare）**。
- **优点：** 如果浅比较发现所有 `props` 都没有变化，React 就会**跳过（Skip）** 这个组件的本次渲染（re-render）及其所有子树的渲染，从而避免了大量不必要的 VDOM-Diff 和 DOM 操作，是提升复杂应用响应速度的关键。
- **缺点：** **浅比较本身是有成本的**。如果一个组件的 `props` 总是会变化，或者组件本身非常轻量，使用它们反而会增加“比较”这一步的额外开销。同时，它只能“浅比较”，如果 `props` 传递了复杂对象，并且父组件中总是创建新对象（即使内容相同），会导致 `memo` 优化失效。

### 使用 useMemo/useCallback

- **原理：** `useMemo` 和 `useCallback` 是 React Hooks 中实现**“记忆化”**的工具。useMemo 缓存一个**计算结果**（值），`useCallback` 缓存一个**函数实例**。它们都接受一个依赖项数组，只有当依赖项变化时，才会重新计算值或创建新的函数实例。
- **优点：** 主要用于配合 `React.memo`。当把一个函数（`useCallback`）或一个对象（`useMemo`）作为 `prop` 传递给一个被 `memo` 的子组件时，它可以保证在父组件重渲染时，传递给子组件的 `prop` 引用地址不变，从而**命中 memo 的浅比较**，防止子组件不必要的重渲染。`useMemo` 也可以用于跳过昂贵的计算。
- **缺点：** **滥用会导致性能下降和代码复杂化**。Memoization 本身需要占用内存来“缓存”结果，并且每次渲染都要检查依赖项数组。如果无脑地把所有函数都用 `useCallback` 包裹，会增加不必要的开销，并可能因为错误的依赖项数组导致“闭包陷阱”或数据陈旧（Stale Data）的 Bug。

### 图片和静态资源优化

- **原理：** 图片资源通常是 Web 页面上“最重”的资产。此优化包含：
  1. **格式选择**（使用 `WebP` 或 `AVIF` 等现代格式）。
  2. **压缩**（在不严重损失画质的前提下减小文件体积）。
  3. **懒加载**（使用 `loading="lazy"` 属性，使视口外的图片在滚动到时才加载）。
  4. **使用 srcset** 提供不同分辨率的图片以适配不同设备。
- **优点：** 极大降低页面总的加载字节数，显著提升加载速度，并节省用户流量。
- **缺点：** 懒加载可能导致图片在滚动时“突然出现”，需要配合占位符或骨架屏来优化体验。图片压缩需要找到质量和体积的平衡点。

### 防抖节流

- **原理：** 针对高频触发的事件（如 `resize`, `scroll`, `input`）进行函数执行频率的控制。**防抖 (Debounce)** 指的是“触发事件后，在 N 秒内没有再次触发，才执行”，常用于搜索框输入。**节流 (Throttle)** 指的是“在 N 秒内，最多只执行一次”，常用于滚动事件监听。
- **优点：** 极大地减少了高频事件处理函数的执行次数，避免了在短时间内大量计算或发送 API 请求，防止了页面卡顿 (Jank) 和资源浪费。
- **缺点：** 会引入轻微的“延迟”感。例如，防抖必须等待用户停止输入，而节流则会“舍弃”掉在时间间隔内的某些事件点，选择合适的延迟时间非常关键。

### 启用 Gzip/Brotli 压缩

- **原理：** 这是一种 **HTTP 传输层**的优化。客户端（浏览器）在请求头中声明 `Accept-Encoding: gzip, brotli`，如果服务器（如 Nginx）配置了压缩，它会在响应前，将文本类资源（JS, CSS, HTML）使用 Gzip 或 Brotli 算法进行压缩，生成一个体积小得多的文件。浏览器接收后再解压。
- **优点：** Brotli 通常比 Gzip 有更高的压缩率（体积更小）。这种方式可以显著减少 60%-80% 的文本资源下载体积，极大提升下载速度。
- **缺点：** 压缩和解压**会消耗少量的 CPU 资源**（服务器的压缩成本和浏览器的解压成本）。但这点消耗相比于网络传输时间的减少几乎可以忽略不计。它只对文本文件有效，对图片、视频等已压缩过的二进制文件无效。

### 虚拟滚动

- **原理：** 当需要渲染一个超长列表（如几千条、几万条数据）时，不把所有列表项全部渲染到 DOM 中。虚拟滚动技术只渲染**当前视口（Viewport）中可见**的列表项（以及上下几条作为缓冲区）。当用户滚动时，它会动态计算并替换视口中的 DOM 元素，模拟出滚动的效果。
- **优点：** 使得渲染“无限”长列表成为可能。无论列表多长，DOM 中始终只有少量（如几十个）节点，保持了极高的渲染性能和内存占用，避免页面崩溃。
- **缺点：** 实现起来相对复杂，需要精确计算项目高度、滚动位置等。如果列表项高度不固定，计算会更复杂。同时，可能会影响页面的“查找”（Ctrl+F）功能和滚动条的准确性（需要用库如 `react-window` 来解决）。

### 浏览器缓存

- **原理：** 通过配置 HTTP 响应头（Response Headers），指示浏览器如何缓存资源。分为**强缓存**（`Cache-Control: max-age=...`）和**协商缓存**（`ETag` / `Last-Modified`）。
- **优点：** 对于强缓存的资源（如带 hash 的 JS/CSS），浏览器在缓存有效期内**完全不会发送网络请求**，直接从本地磁盘读取，速度最快。对于协商缓存的资源（如 `index.html`），浏览器会发送请求，但如果服务器判断资源未变（304 Not Modified），浏览器也只需使用本地副本，节省了下载时间。
- **缺点：** 缓存配置是优化的“双刃剑”。如果配置不当（例如，对入口 `index.html` 设置了过长的强缓存），会导致**应用发布新版后，用户无法及时更新**，看到的是旧的页面或资源错乱。

## HTTP1, HTTP2, HTTP3 的核心演进

这是一个“解决问题”的演进史，核心目标始终是：**更快、更高效**。

### HTTP/1.1

- **核心思想：** 基于文本的“请求-响应”模式。
- **工作机制：**
  - 浏览器为了并发，会“被迫”开启 6-8 个 TCP 连接。
  - **关键问题 (应用层队头阻塞)：** 在**同一个 TCP 连接**上，请求必须“排队”。如果第一个请求（比如一个大图片）非常慢，那么这个连接上的后续请求（比如 CSS/JS）就**必须等待**，即使它们很小。
- **目的/作用：** 这是 Web 发展的基础，但它“慢”且“浪费”连接。

### HTTP/2 (99% 的现代网站)

- **核心思想：** 解决 HTTP/1.1 的“排队”问题。
- **工作机制：**
  - **多路复用：** **所有请求**都运行在**一个 TCP 连接**上。
  - **二进制“流”：** 请求被拆分成“帧”，交错地在同一个连接上传输。
  - **头部压缩 (HPACK)：** 大幅减少重复的 Header。
- **目的/作用 (解决的问题)：**
  - 它完美解决了 HTTP/1.1 的**“应用层”**队头阻塞。100 个请求可以同时发出，互不排队。
- **关键点/陷阱 (暴露的新问题)：**
  - 它暴露了更底层的**“TCP 传输层”队头阻塞**。
  - 因为所有“流”都跑在**一个** TCP 连接上，而 TCP 协议本身要求**严格保序**。如果网络中**丢失了一个 TCP 数据包**，TCP 协议会**强制“暂停”所有流**，等待那个丢失的包被重传。
  - 结果：一个丢包（比如来自图片的流）会卡住所有其他流（比如 JS 和 CSS）。

### HTTP/3

- **核心思想：** 彻底解决“TCP 传输层”的队头阻塞。
- **工作机制：**
  - **抛弃 TCP：** 它不再使用 TCP，而是基于 **UDP** 协议，并在其上构建了一个新的可靠协议：**QUIC**。
  - **QUIC 内置了“流”：** QUIC 的“流”是**真正独立**的。
  - **连接迁移：** 内置加密 (TLS 1.3)，握手更快 (0-RTT)；并且不依赖 IP，切换 WiFi/4G 时连接**不会断**。
- **目的/作用 (解决的问题)：**
  - **彻底解决了队头阻塞**。
  - **场景：** 如果一个“流”（图片）的数据包丢失了，它**只会**阻塞那一个流。其他流（JS, CSS）的数据包可以被正常接收和处理，**完全不受影响**。
- **关键点/陷阱：** QUIC 运行在 UDP 上，这可能在一些老的网络防火墙上遇到支持问题（但这个问题正被快速解决）。
