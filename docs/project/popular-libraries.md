...
一种“局部作用域 CSS”的技术方案（通常基于构建工具实现）。它允许你编写传统的 CSS 文件，但在引入到 JavaScript 时，自动将所有的类名（class）和动画名（animation name）转为局部作用域，从根本上避免全局样式污染。

### **工作机制**

- **特定文件约定：** 通常将样式文件命名为带有特定后缀的文件，如 `*.module.css`（或 `.module.scss` 等预处理器格式）。
- **对象化导入：** 在 JS 文件中，将 CSS 文件像普通模块一样导入（例如 `import styles from './Button.module.css'`）。
- **哈希化编译：** 在构建阶段（如通过 Webpack 或 Vite），工具会将 CSS 中的原始类名编译成全局唯一的哈希字符串（例如 `Button_wrapper__3k9v2`），并在导出的 JS 对象中建立映射。最终输出的是纯静态的 CSS 文件。

### **优点**

- **消除全局作用域：** 同样完美解决了 CSS 的最大痛点——全局命名冲突。你可以放心地在不同组件里重复使用 `container` 或 `btn` 这样的简单类名。
- **零运行时开销：** 这是与许多 CSS-in-JS 方案最大的不同。它在构建时直接生成静态的 `.css` 文件，浏览器直接解析，完全不需要在客户端消耗 JS 性能来生成和注入样式。
- **零学习成本：** 你编写的依然是原汁原味的纯 CSS（或 Sass/Less），完全不需要学习新的 API 或模板语法，迁移旧项目的样式代码也极为方便。
- **利于缓存与加载：** 样式最终剥离为独立的 CSS 文件，浏览器可以并行下载并充分利用缓存机制。

### **关键点/陷阱**

- **动态样式较繁琐：** 相比于 styled-components 可以直接接收 props，CSS Modules 处理动态样式稍显笨拙。你通常需要借助 `classnames` 或 `clsx` 库来根据条件拼接类名，或者结合内联样式（inline-style）来处理动态变量。
- **TypeScript 类型支持弱：** 默认情况下，导入的 `styles` 对象在 TS 中会被视为 `any`。如果你拼错了一个类名，IDE 通常不会报错，也没有代码提示（需要额配置 TS 插件如 `typescript-plugin-css-modules` 才能解决）。
- **依赖构建工具：** 它不是原生的 Web 标准，必须依赖打包工具链的支持（尽管 Next.js、Create React App、Vite 等现代框架都已经将其作为开箱即用的默认配置）。
- **全局作用域混用：** 如果你必须在一个 `.module.css` 文件中去覆盖全局样式，需要使用特殊的语法（如 `:global(.ant-btn) { ... }`），这会让样式代码稍微变得不太纯粹。
